...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MinedCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MinedCoin Signed Message:\n";
...
{
    // MinedCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    int64 nSubsidy = 38 * COIN; //20000000 / 365 / 24 / 60

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    //nSubsidy >>= (nHeight / 840000); // MinedCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // MinedCoin: 1 day
static const int64 nTargetSpacing = 60; // MinedCoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MinedCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MinedCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MinedCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04581EED563676F4BD233C5888B2D5D188658ED16B7A6C04CCDFA7F4983E44DE948CE3178DA286F9448BA85DE09E691687BD04A632B548C625C94DF3A20C5A9895") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1394093000;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // MinedCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MinedCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MinedCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MinedCoinMiner(CWallet *pwallet)
{
    printf("MinedCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("minedcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MinedCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MinedCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MinedCoinMiner, pwallet));
}
